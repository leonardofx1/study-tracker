...
import {
  ...
  ErrorFindSession,
  ErrorUpdateSession,
} from "../../errors/session/session.erros.js";
...
import { sessionSchema } from "../../schemasZod/session.js";

export class SessionController implements ISessionController {
  constructor(
    private createSessionService: CreateSessionService,
    private deleteService: DeleteSessionService,
    private findAllSessionService: FindAllSessionService,
    private findSessionService: FindSessionService,
    private updateSessionService: UpdateSessionService,
  ) {}
    ...
    try {
      ...
      if (error instanceof ErrorDeleteSession) {
        reply
          ...
          .send({
            message:
              "The feature you tried to delete either doesnt exist or has already been removed.",
          });
      }
      throw error;
    }
  ...
  findAllSession = async (req: FastifyRequest, reply: FastifyReply) => {
    try {
      const { idSubject } = req.params as { idSubject: string };
  ...
  update = async (req: FastifyRequest, reply: FastifyReply) => {
    try {
      const session = sessionSchema.parse(req.body)
      const updateRes = await this.updateSessionService.update(
        session as StudySessionDto,
      );
      reply
        .status(200)
        .send({ message: "Study section successfully updated." });
    } catch (error) {
